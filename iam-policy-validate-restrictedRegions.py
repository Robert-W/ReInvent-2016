# This function validates the permissions for users, groups, roles, and policies for the EC2 restricted regions AWS Config Rule
...
# Actions that we will simulate on a specific resource to determine compliance
...
def evaluate_compliance(configuration_item, result_token):
    ...
    if resource_type == "AWS::IAM::User" or resource_type == "AWS::IAM::Role":
        ...
        for group in attached_entities["PolicyGroups"]:
            ...
            simulate_group(group_arn, user["GroupId"], group["GroupName"], config, iam, result_token, timestamp)
...
def record_results(config, compliance_result, result_token, resource_type, resource_id, resource_arn, timestamp):
    ...
    annotation = "Entity: " + resource_arn + " is " + compliance_result + " for validation of restricted EC2 region."
